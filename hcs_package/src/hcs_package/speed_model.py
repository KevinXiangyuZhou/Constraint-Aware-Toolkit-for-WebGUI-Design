...
Provides a common interface for computing desired speed profiles from
geometric features (clearance, curvature, curvature rate).

GAMSpeedModel — learned Generalized Additive Model fitted from
human speed data.  Requires pyGAM (optional dependency).
"""
...
# Additive offsets for log-transform (smooth handling of kappa≈0 in straight tunnels)
_CLEARANCE_EPS = 1e-6
_KAPPA_EPS = 1e-4
_DKAPPA_EPS = 1e-4
...
class GAMSpeedModel:
    ...
    def __init__(self, base_speed: float = 0.13,
        ...
        self.floor = floor
        self.ceil = ceil
        self.gam = None  # fitted pyGAM model
    ...
    def fit(self, clearance: np.ndarray, kappa: np.ndarray,
        ...
        # Determine clearance spline complexity from data support
    ...
    def plot_partial_effects(self, ax=None):
        ...
        n_terms = len(self.gam.terms) - 1  # exclude intercept
        if ax is None:
        ...
        for i in range(min(n_terms, len(axes))):
            ...
            if i < 3:  # univariate terms
                a.plot(np.exp(XX[:, i]), np.exp(pdep))
            else:  # interaction term — just plot the partial dependence
                a.plot(np.exp(pdep))
