...
This module contains the MPCC optimization problem and dynamics matrices.
Speed adaptation is handled externally by a SpeedModel; this module receives
a pre-computed speed profile and solves for optimal jerk controls.

Extracted from model.py during the speed-model refactor.
"""
...
def generate_mpcc(
    ...
    w_corridor = w_constraint          # unified: one fitted weight for all boundary penalties
    w_contour = weights.get('contour', 1.0)
    w_lag = weights.get('lag', 0.1)

    # Use the externally-provided speed profile as the target.
    # Apply light Gaussian smoothing (MPCC-specific post-processing) to prevent
    # the optimizer from chasing step-like changes over the short horizon.
    ...
    # Scale factors for optimization variables
    SCALE_JERK = 1000.0
    ...
    # Optimization variables: x = [jx_0..N-1, jy_0..N-1, vs_0..N-1]
    n_vars = 3 * num_steps
    ...
    # Cartesian constraint active mask
    ...
    # Bounds: vs >= 50% of speed target
    vs_min_per_step = 0.5 * speed_target / SCALE_VS
    bounds = []
    bounds.extend([(None, None)] * num_steps)  # Jx
    bounds.extend([(None, None)] * num_steps)  # Jy
    bounds.extend([(float(vs_min_per_step[i]), None) for i in range(num_steps)])  # Vs
    ...
    # Cache solution for warm-starting the next call
