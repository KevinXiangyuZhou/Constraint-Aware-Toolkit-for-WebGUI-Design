"""
Steering model — thin orchestrator.

Connects a SpeedModel (adaptive speed profile) with the MPCC solver
(trajectory optimisation).  The speed model and MPCC solver live in
their own modules; this file provides the backward-compatible ``model()``
entry point used by CursorSimulator.
...
def model(model_input: SteeringModelInput):
    ...
    # Find current progress along the path
    ...
    # Compute speed profile at estimated horizon positions
    s0_init = theta_0
    s_estimated = s0_init + desired_speed * interval * np.arange(1, pred_horizon + 1)

    # Extract geometric features at the estimated positions by interpolating
